...
Tutto iniziò in chat #haskell.it. f-a chiede aiuto a meditans, il matematico del gruppo, su un esercizio tratto da [Progamming Praxis](http://programmingpraxis.com/2015/10/09/searching-for-hypotenuses/):
...
In tempo record larsen se ne esce con la sua implementazione che fa anche da specifica formale del problema, la chiama con orgoglio "the lower bound of smartness :-)" e ci saluta perché deve andare a pranzare.
...
I tempi di esecuzione sono esponenziali: accettabili per c = 10^3 (4 secondi) e fuori controllo già per c = 10^4
...
Come spesso capita, grazie alla matematica è possibile risolvere un problema apparente complesso, applicando teoremi e sfruttando proprietà dei numeri e riducendo il tutto ad una serie di calcoli decisamente meno onerosa. 
...
Ma quanto paga questa ottimizzazione, rispetto alla soluzione naive di larsen? Beh Pitagora non è entrato nella storia immeritatamente e per 10^3
...
E osare ancora di più a quota 10^7 (10M)
...
Eseguendo un profiling si vede che la stragrande maggioraza del tempo è usato da `triplesLessThan_v1`. Ma alcune (forse molte) delle moltiplicazione eseguite possono essere evitate.

Se si ha `x*a = y*b` dove `a` e `b` sono i fattori co-primi, e `x` e `y`  i fattori 2, 3, ..n, con `x,a,y,b in Int` e `a < b => x > y`, allora `b = a * x/y` e `x/y in Int`, allora l'elemento successivo `(y+1)*b` e` `x*a + x/y*x` che è sicuramente intero ed è sicuramente generato da `a` senza dover iterare su `b`.

Quindi è possibile iterare solo su `a`, ed evitare di iterare su `b`.
...
Ok la matematica paga ancora. Inoltre da un `diff t2.log t3.log` si vede che il risultato è lo stesso, e non abbiamo tagliato delle soluzioni.
...
Il `IntSet` è appunto una struttura dati che memorizza la chiave come un Unboxed Int, ovvero direttamente come Int.
...
Invece di usare la funzione `print` che converte verso una potenzialmente lenta `String`, e confortati dal fatto che il profiler ci dice che una buona parte del tempo è speso per generare la stringa finale con il risultato, riscriviamo l'algo che scrive la string con i ByteString Builder:
...
I tempi per 10^7 passano da 16.5s a 14.9s, con l'unico vantaggio che l'output è personalizzabile rispetto alla built-in function.
...
Molte funzioni accettano o emettono delle liste come result. In un linguaggio con semantica strict, questo sarebbe inefficiente dato che si materializza il risultato in una forma non ottimale, per poi trasformarlo in altro in fase di stampa o processing. Invece in Haskell la lista è un ottimo mezzo per trasferire i dati dato che di fatto si comporta come una coda di dati da elaborare, e il compilatore spesso non la genera nemmeno parzialmente (un pezzo alla volta, in modo incrementale), ma applica direttamente una list fusion fra le funzioni. Ci guadagna il codice che è leggibile, modulare e grazie al compilatore efficiente. Ovvero in C uno deve decidere fin dall'inizio la struttura dati da usare per far comunicare fra loro le funzioni ed è difficile scrivere codice che si comporta come se fosse un generator/iterator e produce un chunk alla volta. Si può fare ma il codice risultante è un blocco unico, poco modulare e poco riusabile.

Quindi in Haskell si scrivono tante funzioni riusabili in diversi contesti, e ci pensa il compilatore a compilarle come se fossero un blocco di codice unico e coeso. Sembra poco, ma in realtà è molto dato che viceversa uno deve riscrivere e rifattorizzare più volte lo stesso codice, per tenere conto di piccole differenze di formato dati.

Low Level Optimization: Bit Array
---------------------------------

I risultati sono densi rispetto ai numeri interi: ovvero dato un insieme di numeri [1 .. n], sono molti i numeri che stanno nell'insieme. In questo caso un Bit Vector in cui viene usato un Bit 0/1 per indicare se un certo Int è parte della soluzione o no risulta essere efficiente, dato che usa meno RAM rispetto alla memorizzazione esplicita di tutti i numeri. Per dire, 1M di Int a 64 bit, occupa 4MB di RAM, mentre usando i Bit Vector 122KB di RAM.

Haskell supporta Array unboxed (i valori sono memorizzati nella cella di memoria senza passare per un puntatore e relativa deference) e aggiornabili in modo Mutable (con modifica in-place, senza salvare il vecchio valore), come si ha con gli array C. Le operazioni mutable devono essere eseguite all'interno di una Monad ad-hoc. La monad permette di esprimere algoritmi di tipo mutable/imperative, come si farebbe in C. Al termine dei calcoli, il valore tornato dalla Monad torna ad essere un value puro gestibile da Haskell e dalle funzioni e quindi si "snatura" il tutto solo all'interno della Monad o delle funzioni chiamate all'interno della Monad, perché ricordiamo che una Monad può chiamare altre Monad dello stesso tipo.

Da notare come questo comportamento è in linea con la filosofia Haskell che è più quella di usare il Domain Specific Language giusto per risolvere un problema, che quella di usare sempre e comunque funzioni pure. E alcuni algoritmi sui grafi o altro, risultano più chiari usando mutable references. In questo caso usiamo una Mutable monad principalmente per motivi di efficienza.
...
TODO applicare ottimizzazione dei fattori ripetuti come fatto nei passi prima e vedere la differenza di velocità.

Librerie a Disposizione
-----------------------

Haskell è un linguaggio poco usato, ma nonostante ciò vi è un alto numero di librerie che si possono usare, ed è difficile non trovare quello di cui si ha bisogno. Nel nostro caso:
